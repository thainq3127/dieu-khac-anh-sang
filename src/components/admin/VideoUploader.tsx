...
export default function VideoUploader({ value, onChange, placeholder, className, id }: VideoUploaderProps) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Đã có lỗi xảy ra khi tải video lên.')
    } finally {
      setUploading(false)
    ...
    <div className={className} id={id}>
      ...
      {value ? (
        <div className="flex items-center gap-3 p-2.5 border border-input rounded-lg bg-muted/10">
          <div className="relative w-12 h-12 rounded overflow-hidden border bg-muted shrink-0 flex items-center justify-center">
            <video
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-emerald-600 font-semibold uppercase tracking-wider mt-0.5">Video đã tải lên</p>
          </div>
